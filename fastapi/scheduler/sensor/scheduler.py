...
class SensorScheduler:
    ...
    BASE_URL = "https://api-wmumpxg2lq-du.a.run.app/api/device-data"
    ...
    def _get_or_create_device_by_identity( # 이건 나중에 인프라에 넣을까?
        ...
        if device:
            # 기존 디바이스가 있고 extra_data가 전달되면 즉시 업데이트
            if extra_data:
    ...
    def fetch_api_data(self, start_dt: datetime, end_dt: datetime, limit: int = 50000) -> list:
        ...
        try:
            ...
            if response.status_code == 200 and response.text.strip():
                ...
                data = response.json()
                self.logger.info(f"API 데이터 수신 성공: {len(data)}개 항목")
                return data
            else:
                # HTTP 오류이거나 빈 응답이면 실패 처리
                self.logger.error(f"API 요청 실패: {response.status_code}")
                return []
                
        except Exception as e:
            # 네트워크/파싱 등 예외 발생 시 빈 리스트 반환
            self.logger.error(f"API 요청 중 에러 발생: {e}")
            return []
    
    def classify_data(self, data: list) -> tuple: # 이건 나중에 인프라에 넣을까?
        """
        ...
        self.logger.info(f"데이터 분류 완료 - AI_PEMS: {len(ai_pems_data)}, PEMSPRO: {len(pemspro_data)}, PEMSPROPLUS: {len(pemsproplus_data)}, FLOW: {len(flow_data)}")
        return ai_pems_data, pemspro_data, pemsproplus_data, flow_data
    
    def _parse_datetime(self, dt_str: any) -> datetime: # 이건 나중에 인프라에 넣을까?
        """
    ...
    def process_pemspro(self, pemspro_data: list) -> int:
        ...
        for item in pemspro_data:
            try:
                ...
                # PEMS_PRO 모델에는 명시적 log_dt가 없을 수 있으므로 중복 체크는 하지 않음
                pems_log_data = {
                    'device_id': device_id,
                    'log_dt': log_dt,
