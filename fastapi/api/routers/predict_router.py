...
from api.schemas.responses import DevicePredictionResponse, ManualPredictResponse, PredictBatchResponse
...
@router.get("/predict/{device_id}", response_model=DevicePredictionResponse)
def predict_by_device(device_id: str, lookback_hours: int = 24, max_data_age_hours: int = 24):
    ...
    try:
        result = predict_one_device(device_id, lookback_hours, max_data_age_hours)
...
def predict_batch(req: MultiPredictRequest):
    ...
    for device_id in req.device_ids:
        try:
            result = predict_one_device(device_id, req.lookback_hours, req.max_data_age_hours)
